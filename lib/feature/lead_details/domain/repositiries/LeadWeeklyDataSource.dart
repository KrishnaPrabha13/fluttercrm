...
class LeadWeeklyDataSource extends DataGridSource {
  LeadWeeklyDataSource(BuildContext context,
      {required List<WeeklyLeadListModel> crmLeadData}) {
    ...
    _leadsData = crmLeadData
        .map<DataGridRow>((e) => DataGridRow(cells: [
              ...
              DataGridCell<String>(columnName: 'date&time', value: e.dayName),
              DataGridCell<String>(columnName: 'country', value: e.country),
              DataGridCell<String>(
                  columnName: 'lookingfor', value: e.leadlookingfor),
              DataGridCell<String>(
                  columnName: 'probabilityofleadcloser',
                  value: e.probailityofleadcloser),
              DataGridCell<String>(
...
/* @override
  DataGridRowAdapter buildRow(DataGridRow row) {
    return DataGridRowAdapter(cells: <Widget>[
      ...
      Container(
        alignment: Alignment.center,
        child: ((row.getCells()[1].value.toString().trim() == '-') ||
                (row.getCells()[1].value.toString() == ''))
            ? const Text(
                'N/A',
              )
      ...
      Container(
        alignment: Alignment.center,
        child: Text(getCTPDateTime(row.getCells()[4].value.toString())!),
      ),
      Container(
          alignment: Alignment.center,
          child: Container(
            ...
            height: 32,
            decoration: ShapeDecoration(
              color: getLeadColor(row.getCells()[5].value.toString()),
              shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(2),
              ),
            ),
            child: Center(
              child: Text(
                ...
                style: TextStyle(
                    color: getTextColor(row.getCells()[5].value.toString()),
                    fontFamily: FontName.montserrat,
                    fontSize:
                        (Responsive.isDesktop(originalContext!)) ? 13 : 11),
              ),
  ...
  Widget getLeadStatusImage(DataGridRow row) {
    return SvgPicture.asset(
      row.getCells()[5].value.toString() == "Contacted"
          ? lead_contacted
          : row.getCells()[5].value.toString() == "Not Contacted"
              ? lead_notcontacted
              : row.getCells()[5].value.toString() == "Pre-Qualified"
                  ? lead_prequalified
                  : row.getCells()[5].value.toString() == "Not Qualified"
                      ? lead_notqualified
                      : row.getCells()[5].value.toString() ==
                              "Attempted to Contact"
                          ? lead_attemptedtocontact
                          : row.getCells()[5].value.toString() == "Lost Lead"
                              ? lead_lostlead
                              : lead_junk,
      height: 30,
      width: 30,
