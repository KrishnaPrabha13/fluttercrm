...
import 'package:crm_dashboard/feature/lead_details/domain/repositiries/LeadWeeklyDataSource.dart';
...
class LeadWeeklyDetails extends StatefulWidget {
  ...
  LeadWeeklyDetails(
      ...
      : super(key: key);
  @override
  State<LeadWeeklyDetails> createState() => LeadWeeklyDetailsState();
}

class LeadWeeklyDetailsState extends State<LeadWeeklyDetails> {
  final FirebaseLeadServices _firebaseLeadServices = FirebaseLeadServices();
  List<GridColumn>? columns;
  LeadWeeklyDataSource? leadWeeklyDataSource;
  @override
  void initState() {
    leadTabIndex = 2;
    super.initState();
  ...
  Widget build(BuildContext context) {
    ...
    leadWeeklyDataSource?.sortedColumns.add(const SortColumnDetails(
        name: 'name', sortDirection: DataGridSortDirection.ascending));
    leadWeeklyDataSource?.sort();
    return Scaffold(
      resizeToAvoidBottomInset: false,
      body: FutureBuilder<List<WeeklyLeadListModel>>(
        future: _firebaseLeadServices.getWeeklyLeads(
            context, widget.searchQuery, getAccess),
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            List<WeeklyLeadListModel>? data = snapshot.data;
            if (data!.isEmpty) {
              return Center(
                child: Column(
                  children: [
            ...
            leadWeeklyDataSource =
                LeadWeeklyDataSource(context, crmLeadData: data.toList());
                  ...
                  child: SfDataGrid(
                    ...
                    onCellTap: ((details) {
                      if (details.rowColumnIndex.rowIndex != 0) {
                        ...
                        var row = leadWeeklyDataSource!.effectiveRows
